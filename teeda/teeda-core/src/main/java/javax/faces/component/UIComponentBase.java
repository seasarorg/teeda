...
public abstract class UIComponentBase extends UIComponent {
	...
	public String getClientId(FacesContext context) {
		...
		while ((component = component.getParent()) != null) {
			if(component instanceof NamingContainer) {
				parentId = component.getClientId(context) + NamingContainer.SEPARATOR_CHAR;
				break;
			}
		}
	...
	private static UIComponent getComponentRoot(UIComponent base) {
		UIComponent parent;
		for (parent = base.getParent(); parent.getParent() != null;
	...
	public Object processSaveState(FacesContext context) {
		...
		return new SerializableStateHolder(saveState(context), facetMap, children);
	}
	...
	public static Object saveAttachedState(FacesContext context, Object attachedObject){
		ComponentUtils_.assertNotNull(context, "context");
		ComponentUtils_.assertNotNull(attachedObject, "attachedObject");
	...
	public static Object restoreAttachedState(FacesContext context, Object stateObject){
		...
		if(stateObject instanceof List){
			...
			for(Iterator itr = list.iterator();itr.hasNext();){
				...
				AttachedObjectStateWrapper_ wrapper = (AttachedObjectStateWrapper_)itr.next();
	...
	private Object saveAttributesMap(){
		return (attributesMap_ != null) ? attributesMap_.getAttributesActual() : null;
	}
	...
	private Object saveValueBindingMap(FacesContext context){
		...
		if(bindingMap_ != null){
			states = new HashMap();
			for(Iterator itr = bindingMap_.entrySet().iterator();itr.hasNext();){
				Map.Entry entry = (Map.Entry)itr.next(); 
				states.put(entry.getKey(), saveAttachedState(context, entry.getValue()));
			}
		}
		return states;
	}
	...
	private void restoreValueBindingMap(FacesContext context, Object state){
		...
		if(state != null){
			...
			for(Iterator itr = stateMap.keySet().iterator();itr.hasNext();){
				Map.Entry entry = (Map.Entry)itr.next();
				bindingMap_.put(entry.getKey(), restoreAttachedState(context, entry.getValue()));
			}
		}
